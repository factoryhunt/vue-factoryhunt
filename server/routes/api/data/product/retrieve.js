...
router.get('/domain/:input', (req, res) => {
  const domain = req.params.input.toLowerCase()
  retrieveSingleProductByProductDomain(domain)
    .then((result) => {
      res.status(200).send(result[0])
    })
    .catch(() => {
...
function retrieveSingleProductByProductDomain (domain) {
  return new Promise((resolve, reject) => {
    mysql.query(`SELECT * FROM ${CONFIG_MYSQL.TABLE_PRODUCTS} WHERE lower(product_domain) = '${domain}'`,
      (err, rows) => {
        if (err) return reject(err)
        resolve(rows)
      })
